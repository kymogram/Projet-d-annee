...
def main():
    ...
    parser.add_argument("-s", default = nom_pers[alea_indice], help = "Nom " + \
                        "de la personne initiant la rumeur")
    parser.add_argument("-r", type=int, default = alea_init, help = \
                        "valeur de la rumeur initiale")
    parser.add_argument("-p", type=float, default = 0.1, help = \
                        "Probabilité qu'une rumeur soit modifiée " + \
                        "lorsqu'elle est racontée (réel entre 0 et 1)")
    parser.add_argument("-t", type=int, default = nbr_simu, help = "Nombre " + \
                        "d'étape dans la simulation")
    parser.add_argument("-d", default = False, help = "Si cette option est " + \
                        "choisie, chaque personne connaissant la rumeur la " + \
                        "transmet à un ami choisi aléatoirement parmi " + \
                        "ceux ne connaissant pas déjà la rumeur", \
                        action = "store_true")
    parser.add_argument("-m", type=str, default = "none", \
                        help = "Type de modification éventuelle de la rumeur " + \
                        "lorsqu'elle est racontée (choix : incremental, " + \
                        "bitflip, none)")
